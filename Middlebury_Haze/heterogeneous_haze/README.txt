...
IMPORTANT: While we have selected  images for the HazeRD dataset that have depth maps that are well aligned with the  RGB images, there are occasional
registration errors between the RGB and depth images caused by transparency of window glass panes, fine structure in trees, moving objects, etc. These
regisration errors cause minor errors in simulated hazy images which can be seen in the corresponding regions. We are working on techniques for reducing
such misregistration and artifacts and in the event that we make a substantial improvement, may provide an update to HazeRD via the above mentioned
website.
...
|   |-- depth
|   |   |-- contains depth maps (mat files)
|   |-- img
|   |   |-- contains color images (jpg files)
|   |-- simu
