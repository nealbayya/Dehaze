...
save_dir = 'data/simu_diff_alpha/';

addpath('./utils');

% set parameters
pert_perlin = 0;    % 1 for add perlin noise
airlight_param = [0.5,0.6,0.7,0.9,1.1];

for k =1:length(airlight_param)
	airlight = airlight_param(k)*ones(3,1);  % airlight in the range [0,1]
	visual_range = [0.05,0.1,0.2,0.5]; % visual range in km
