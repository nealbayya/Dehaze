function [fnames, beta_values] = hazy_simu_regression(img_name,depth_name,save_dir,varargin)
...
fnames = cell(length(visual_range), 1);
beta_values = zeros(length(visual_range), 1);

for i=1:length(visual_range)
    VR = visual_range(i);
...
%	map, which is equivalent to change the optic distance. 
    ...
    save_name = strrep(name,'_RGB',['_',num2str(i),ext]);
    fnames{i} = save_name;
    imwrite(I2,[save_dir,save_name]);
    %[bar,name,ext] = fileparts(depth_name); 
    %save_name_trans = strrep(name,'_depth',['_trans_',num2str(i),ext]);
