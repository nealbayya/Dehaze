...
for VR = visual_range
    ...
    %I0 = im2double(imread(img_name)); %octave does not support im2double
    I0 = double(imread(img_name)) / 255.0; %insertion by NRB - 09/14/18
...
%	map, which is equivalent to change the optic distance. 
    if pert_perlin
        d = d.*((perlin_noise(zeros(size(d)), 0.1)-0.5)+1); % Decay parameter modified by NRB,09/19/18
    end
    ...
    save_name = strrep(name,'_RGB',['_',num2str(VR*1000), '_', num2str(A(1,1,1)*100),ext]);
